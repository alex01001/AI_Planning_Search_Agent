...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for a in self.all_actions:
            actionNode = PgNode_a(a)
            if actionNode.prenodes.issubset(self.s_levels[level]):
                actionNodes.append(actionNode)
                for sNnode in self.s_levels[level]:
                    sNnode.children.add(actionNode)
                    actionNode.parents.add(sNnode)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # checking interference with positive preconditions
        ...
        # checking interference with negative preconditions
        for effect in node_a1.action.effect_add:
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for s1Precond in node_s1.parents:
            for s2Precond in node_s2.parents:
                if(not s1Precond.is_mutex(s2Precond)):
                    return False
